...
4. Copy the `.env.example` file to a new file named `.env` and fill in the placeholders with the appropriate client ID and client secret. You can do this by running ```cp .env.example .env``` and ```cp backend/.env.example backend/.env```
